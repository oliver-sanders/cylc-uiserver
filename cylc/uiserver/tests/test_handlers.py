...
def _create_handler(app, logged_in=True):
...
@pytest.fixture
def uis_subscription_handler_cls(cylc_uis):
    for handler in cylc_uis.handlers:
        if handler[0] == 'cylc/subscriptions':
            return handler[1]


def test_websockets_subprotocol(uis_subscription_handler_cls):
    # handler = _create_handler(cylc_uis)
    assert uis_subscription_handler_cls.select_subprotocol(
        uis_subscription_handler_cls,
        subprotocols=[]
    ) == GRAPHQL_WS


# @pytest.mark.usefixtures("auth_yossarian")
# def test_websockets_check_origin_accepts_same_origin(uis_subscription_handler_cls, jp_serverapp, cylc_uis):
...
# 
#     breakpoint()
#     handler = uis_subscription_handler_cls(request)
# 
#     assert handler.check_origin(uis_subscription_handler, f'http://{host_header}')
